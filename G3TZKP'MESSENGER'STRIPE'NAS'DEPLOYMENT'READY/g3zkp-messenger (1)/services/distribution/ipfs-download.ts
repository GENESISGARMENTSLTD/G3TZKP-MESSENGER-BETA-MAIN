...
export interface DownloadProgress {
  ...
  eta: number; // seconds
  etaFormatted: string;
  status: 'idle' | 'connecting' | 'downloading' | 'verifying' | 'complete' | 'error';
  error?: string;
}
 ...
 */
const DOWNLOAD_PACKAGES: Record<Platform, DownloadPackage> = {
  windows: {
    platform: 'windows',
    version: CURRENT_VERSION,
...
export class IPFSDownloadService {
   ...
   */
  getPackage(platform: Platform): DownloadPackage {
   ...
   */
  getAllPackages(): DownloadPackage[] {
   ...
   */
  async findFastestGateway(): Promise<string> {
   ...
   */
  cancelDownload(platform: Platform): void {
    const downloadId = Array.from(this.activeDownloads.keys()).find(
      id => id.startsWith(platform)
    );
   ...
   */
  async verifyIntegrity(blob: Blob, expectedHash: string): Promise<boolean> {
    try {
   ...
   */
  getDirectDownloadUrl(platform: Platform): string {
    const pkg = DOWNLOAD_PACKAGES[platform];
   ...
   */
  getMagnetLink(platform: Platform): string {
   ...
   */
  getIPFSCID(platform: Platform): string {
   ...
   */
  getInstallInstructions(platform: Platform): string[] {
    switch (platform) {
      case 'windows':
        return [
