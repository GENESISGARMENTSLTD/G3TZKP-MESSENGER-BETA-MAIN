...
export class StripeGateway {
   ...
   */
  async createCheckoutSession(options: CheckoutOptions = {}): Promise<PaymentSession> {
   ...
   */
  async verifyPayment(sessionId: string): Promise<PaymentSession> {
   ...
   */
  private async generateDisplayLicenseKey(license: any): Promise<string> {
   ...
   */
  private persistSession(session: PaymentSession): void {
    localStorage.setItem(
      `g3zkp_payment_session_${session.id}`,
