...
import { ipfsDownloadService, licenseManager, type DownloadProgress } from '../services';
...
const Download: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (userAgent.indexOf('iphone') !== -1 || userAgent.indexOf('ipad') !== -1) setPlatform('ios');
    ...
    <div className="max-w-4xl mx-auto px-6 py-20">
      <div className="text-center mb-16">
        <div className="inline-block mb-4 px-3 py-1 border border-primary/20 text-primary text-[8px] font-mono tracking-[0.5em] uppercase">
          VERSION_{releaseInfo.version} // {releaseInfo.releaseDate}
        </div>
        <h1 className="text-4xl font-display font-black mb-4 tracking-tighter uppercase glow-cyan">Download G3ZKP Messenger</h1>
        <p className="text-white/50 text-sm font-mono">Securely delivered via IPFS & libp2p. SHA-256 verified distributions.</p>
      </div>

      {!hasLicense && (
        <div className="mb-12 p-6 spatial-glass border-yellow-500/30 bg-yellow-500/5 rounded-lg text-center">
          <p className="text-yellow-500 font-mono text-sm mb-4">
            ⚠️ LICENSE_REQUIRED — Purchase a license to unlock full features
          </p>
          <button
            onClick={() => window.location.hash = '#/pricing'}
            className="px-8 py-3 bg-yellow-500 text-dark font-black text-sm tracking-widest uppercase hover:brightness-110 transition-all"
          >
            GET_LICENSE
          </button>
        </div>
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-12">
        {platforms.map((p) => (
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 font-bold transition-all border ${
              platform === p.id 
          ...
          >
            {p.icon}
            <span className="text-xs tracking-widest uppercase">{p.label}</span>
          </button>
        ))}
      </div>

      {/* Download Card */}
      <div className="spatial-glass p-12 rounded-lg border-primary/20 bg-primary/5 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center">
          <div className="w-24 h-24 bg-primary/10 border-2 border-primary/40 rounded-sm flex items-center justify-center mb-8 relative">
            ...
            <svg className="w-10 h-10 text-primary drop-shadow-[0_0_10px_rgba(0,136,255,0.8)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
            </svg>
          </div>
          
          <h2 className="text-3xl font-display font-black mb-2 tracking-[0.2em] text-white uppercase">
            {pkg.filename}
          </h2>
          <p className="text-primary/60 font-black text-xs mb-6 tracking-[0.4em]">
            SIZE: {pkg.sizeFormatted} // PLATFORM: {platform.toUpperCase()}
          </p>
          ...
          {!downloading ? (
            <div className="space-y-4 w-full max-w-md">
              <button
                ...
                className="w-full px-16 py-6 bg-primary text-white font-black text-xl skew-x-[-12deg] hover:brightness-110 transition-all shadow-[0_0_40px_rgba(0,136,255,0.3)]"
              >
                <span className="block skew-x-[12deg]">
                  {platform === 'ios' ? 'OPEN_PWA' : 'DOWNLOAD_VIA_IPFS'}
                </span>
              </button>
              ...
              {platform !== 'ios' && (
                <div className="flex gap-4">
                  <button
                    ...
                    className="flex-1 py-3 border border-white/10 text-white/60 font-mono text-xs tracking-widest hover:border-white/30 hover:text-white transition-all uppercase"
                  >
                    Direct Link
                  </button>
                  {pkg.magnetLink && (
            ...
            <div className="w-full max-w-md space-y-6">
              <div className="text-center mb-4">
                <span className="text-primary font-mono text-sm uppercase tracking-widest">
                  {progress?.status === 'connecting' && 'Connecting to IPFS network...'}
                  {progress?.status === 'downloading' && 'Downloading...'}
                  {progress?.status === 'verifying' && 'Verifying integrity...'}
                  {progress?.status === 'complete' && 'Download complete!'}
                  {progress?.status === 'error' && `Error: ${progress.error}`}
                </span>
              </div>
              <div className="h-2 bg-white/5 overflow-hidden border border-white/10">
                <div 
              ...
              <div className="flex justify-between text-[10px] font-black text-secondary uppercase tracking-[0.3em]">
                ...
                <span>{progress?.speedFormatted} • ETA: {progress?.etaFormatted}</span>
              </div>
            </div>
          )}
          ...
          <div className="mt-12 pt-8 border-t border-white/5 w-full">
            <div className="grid grid-cols-2 gap-4 text-[10px] font-mono text-white/40">
              <div>
                <span className="text-white/20 uppercase">IPFS CID:</span>
                <div className="text-primary/60 break-all mt-1">{pkg.ipfsCID}</div>
              </div>
              <div>
                <span className="text-white/20 uppercase">SHA-256:</span>
                <div className="text-secondary/60 break-all mt-1">{pkg.sha256}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-12 spatial-glass p-8 rounded-lg border-white/5">
        <h3 className="text-lg font-display font-black mb-6 text-white uppercase tracking-widest">
          Installation Instructions
        </h3>
        <ol className="space-y-3">
          {instructions.map((step, i) => (
            <li key={i} className="flex gap-4 text-white/60 text-sm font-mono">
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="spatial-glass p-8 rounded-lg border-white/5">
          ...
          <p className="text-sm text-white/50 font-mono">
            Downloads are served via decentralized IPFS network. Your download helps other users get the app faster and more securely.
          </p>
        </div>
        <div className="spatial-glass p-8 rounded-lg border-white/5">
          <h3 className="text-lg font-bold mb-3 flex items-center gap-2 text-secondary">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-sm text-white/50 font-mono">
            Every binary is SHA-256 verified. The IPFS CID is immutable. You are guaranteed to run the exact code we published.
          </p>
        </div>
      </div>
    </div>
